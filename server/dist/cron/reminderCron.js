...
const sendEmailNotification = (email, subject, text, html) => __awaiter(void 0, void 0, void 0, function* () {
    const mailOptions = {
...
const processTaskNotification = (task, user) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (user.fcmToken) {
            try {
                ...
                const message = `Reminder: "${task.title}" is due ${task.dueDate.toLocaleString()}`;
                yield (0, notificationUtils_1.sendPushNotification)(user.fcmToken, title, message, {
  ...
  <div class="email-container">
    ...
    <div class="content">
      ...
      <div class="task-card">
        ...
        <p class="task-details">
          <span class="label">Due:</span> ${new Date(task.dueDate).toLocaleString()}
        </p>
        
        <p class="task-details">
          <span class="label">Priority:</span> 
          <span class="priority-badge $<span class="priority-badge ${priorityClass}">${task.priority}</span>
        </p>
    ...
    <div class="footer">
      ...
      <p>This is an automated reminder from your TaskWise application.</p>
        ...
        const emailSent = yield sendEmailNotification(user.email, emailSubject, emailText, emailHtml);
        return notificationSent || emailSent;
    }
    catch (error) {
        console.error(`Error processing notifications for task ${task._id}:`, error);
        return false;
    }
});
...
node_cron_1.default.schedule("* * * * *", () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const lookAheadTime = (0, date_fns_1.addMinutes)(currentTime, 15); // Look ahead 15 minutes
    try {
        // Find relevant tasks
        ...
        for (const task of tasks) {
            try {
                ...
                const notificationSuccess = yield processTaskNotification(task, user);
                if (notificationSuccess) {
                    // Mark notification as sent only if at least one notification method succeeded
                    yield Task_1.default.findByIdAndUpdate(task._id, {
                        notificationSent: true,
                    });
                }
            }
